...
export class Logger {
  ...
  debug(message: any): void {
    const { method, filename, line } = this.getStack()!;
    Zotero.debug(
      `${this._outPrefix()} [${config.addonName}] ${filename}:${line}@${method}: ${message}`,
    );
  }
  ...
  getStack() {
    const array = new Error().stack!.split("\n");
    let method, filename, line;
    for (let index = 0; index < array.length; index++) {
      ...
      // }
      filename = this._parseRegExpGroup(/.*\/(.*?js)\:.*$/, element); //eslint-disable-line
      ...
      line = this._parseRegExpGroup(/(\d*)\:\d*$/, element); //eslint-disable-line
      return { method, filename, line };
    }
  }
